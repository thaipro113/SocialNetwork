...
// Helper to get my avatar
function getMyAvatar() {
    const headerImg = document.querySelector('header img.object-cover');
    return headerImg ? headerImg.src : '/Uploads/default-avatar.png';
}
...
async function initiateVideoCall(conversationId, otherUserId, isGroup = false, audioOnly = false) {
    ...
    try {
        ...
        // Show My Avatar
        document.getElementById("localAvatar").src = getMyAvatar();
        ...
        document.getElementById("callStatusText").textContent = "Đang chờ biến người dùng phản hồi...";
...
function addRemoteVideo(userId, stream) {
    ...
    // video.autoplay = true; // Use promise
    ...
    // Vì track disabled không dễ detect ngay, ta dựa vào state?
    // Hiện tại: Nếu state audioOnly=true, ta hiện avatarOverlay. 
    // Nếu người dùng toggle, ta cần update.
    // Simplification: Always add overlay, toggle visibility.
    // But how to detect visibility? 
    // For initial Audio Call sync:
    let showAvatar = false;
    if (incomingCallData && incomingCallData.audioOnly) {
        showAvatar = true;
    }
    ...
    if (videoTrack) {
        // Initial check: If audioOnly, we showed overlay.
        // If track is muted initially (browser dependent implementation), ensure overlay is correct.
...
function toggleCamera() {
    ...
    // TODO: Send specific signal to remote to toggle their overlay if needed, 
    // currently relying on 'black frame' or track mute might not auto-show the overlay on remote side
    // without track tracking logic.
    // For this prototype, I will just control local UI.
    updateCameraButton();
}
// Bật/tắt microphone
