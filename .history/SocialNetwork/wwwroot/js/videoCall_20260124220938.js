...
function createPeerConnection(targetUserId) {
    ...
    pc.oniceconnectionstatechange = () => {
        ...
        if (state === "connected" || state === "completed") {
            ...
            if (statusHeader) statusHeader.textContent = "Đang gọi video";
...
async function endVideoCall() {
    // End for everyone (if caller) or just leave (if participant)
    // For simplicity, we send EndVideoCall with 0 if we initiated, or callerId if we accepted?
    // Actually ChatHub handles 0 as "End for everyone".
...
if (typeof chatConnection !== 'undefined') {
    ...
    chatConnection.on("IncomingVideoCall", (data) => {
        ...
        if (isCallActive) {
            // Đang bận -> Tự động từ chối (hoặc hiện thông báo waiting)
            // Ở đây ta tạm thời bỏ qua hoặc reject
    ...
    chatConnection.on("VideoCallAccepted", async (data) => {
        ...
        // Bắt đầu quy trình WebRTC
        // Tạo PeerConnection với người nhận
    ...
    chatConnection.on("VideoCallRejected", (data) => {
        ...
        if (isGroupCall) {
            ...
            if (expectedParticipants > 0 && rejectedCount >= expectedParticipants) {
                // TẤT CẢ TỪ CHỐI → TẮT NGAY LẬP TỨC
            ...
            cleanupCall(); // ← Cho phép gọi lại ngay!
        }
    });
    ...
    chatConnection.on("VideoCallEnded", (data) => {
        ...
        // Nếu đang trong cuộc gọi → hiện modal kết thúc
    ...
    // 5. Nhận Offer (bên nhận cuộc gọi)
    chatConnection.on("ReceiveOffer", async (data) => {
        ...
        try {
            ...
            // Xử lý các ICE candidates còn tồn đọng (nếu có)
    ...
    chatConnection.on("ReceiveIceCandidate", async (data) => {
        ...
        if (pc) {
            if (pc.remoteDescription) {
                ...
                // Remote description chưa sẵn sàng, lưu lại để add sau
                if (!pendingCandidates[data.fromUserId]) {
    ...
    // 8. User Left Call (Participant left)
    chatConnection.on("UserLeftCall", (data) => {
        ...
        const statusText = document.getElementById("callStatusText");
        if (statusText) {
            // statusText.textContent = `User ${data.userId} left the call`;
...
// Kiểm tra và ẩn/hiện nút video call dựa trên hỗ trợ
function updateVideoCallButton() {
    const videoCallBtn = document.getElementById("videoCallBtn");
    if (videoCallBtn) {
        if (checkMediaSupport()) {
            // Hiển thị nút nếu hỗ trợ
            videoCallBtn.style.display = "flex";
            videoCallBtn.title = "Gọi video";
        } else {
            // Ẩn nút hoặc disable nếu không hỗ trợ
            videoCallBtn.style.display = "none";
            // Hoặc có thể disable và thay đổi title:
            // videoCallBtn.disabled = true;
            // videoCallBtn.title = "Gọi video chỉ hoạt động trên HTTPS";
        }
    }
}
...
// Override function updateVideoCallButton nếu có
if (typeof window.updateVideoCallButtonOriginal === 'undefined') {
    window.updateVideoCallButtonOriginal = window.updateVideoCallButton || function () { };
}

// Function để gọi từ chat.js
window.checkVideoCallSupport = function () {
    updateVideoCallButton();
};