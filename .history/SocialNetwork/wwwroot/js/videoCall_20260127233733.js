...
let userCameraStates = {}; // { userId: boolean } (true = ON, false = OFF)
...
function addRemoteVideo(userId, stream) {
    ...
    // 1. Check Explicit Camera State (highest priority)
    if (typeof userCameraStates[userId] !== 'undefined') {
        if (!userCameraStates[userId]) {
            showAvatar = true;
        }
    }
    // 2. Check Audio Mode
    else if (typeof currentCallIsAudioOnly !== 'undefined' && currentCallIsAudioOnly) {
        showAvatar = true;
...
if (typeof chatConnection !== 'undefined') {
    ...
    chatConnection.on("VideoCallAccepted", async (data) => {
        ...
        // RE-SEND CAMERA STATE IF NEEDED (For Host)
        // If my camera is OFF (and it's not strictly audioOnly mode where it's implied),
        // I need to tell the new participant that my camera is OFF.
        if (!cameraEnabled && !currentCallIsAudioOnly) {
            setTimeout(() => {
                if (callConversationId)
                    chatConnection.invoke("ToggleCamera", parseInt(callConversationId), cameraEnabled)
                        .catch(e => console.log("Error re-sending camera state:", e));
            }, 1000); // Wait a bit for connection
        }
    });
    ...
    chatConnection.on("ReceiverToggleCamera", (data) => {
        ...
        // Store state to handle race condition (Event before Video Element)
        userCameraStates[data.userId] = data.isEnabled;
