...
namespace SocialNetwork.Controllers
{
    public class PostController : Controller
    {
        ...
        public async Task<IActionResult> Comment([FromBody] CommentDto dto)
        {
            ...
            // 2. Gửi thông báo cho người được trả lời (nếu có ParentCommentId)
            if (dto.ParentCommentId.HasValue)
            {
                var parentComment = await _context.Comments.FindAsync(dto.ParentCommentId.Value);
                if (parentComment != null && parentComment.UserId != userId) // Không tự notify chính mình
                {
                    string msg = $" đã trả lời bình luận của bạn: {dto.Content}";
                    var noti = await CreateNotification(parentComment.UserId, userId.Value, dto.PostId, "reply", msg); // Type = reply
                    await SendNotificationToClient(parentComment.UserId, noti, user);
                }
            }
