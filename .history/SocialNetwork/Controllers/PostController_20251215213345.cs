...
namespace SocialNetwork.Controllers
{
    public class PostController : Controller
    {
        ...
        public async Task<IActionResult> Comment([FromBody] CommentDto dto)
        {
            ...
            // Handle Mention Tokenization
            if (dto.TargetUserId.HasValue)
            {
                var targetUser = await _context.Users.FindAsync(dto.TargetUserId.Value);
                if (targetUser != null && !string.IsNullOrEmpty(targetUser.FullName))
                {
                    string mentionPrefix = "@" + targetUser.FullName;
                    if (dto.Content.StartsWith(mentionPrefix)) // Simple prefix check
                    {
                        // Convert to token: {{mention:ID:Name}}
                        // We use substring to preserve the rest of the message (including the space usually after name)
                        string rest = dto.Content.Substring(mentionPrefix.Length);
                        dto.Content = $"{{{{mention:{targetUser.UserId}:{targetUser.FullName}}}}}" + rest;
                    }
                }
            }
            ...
            // 1. Gửi thông báo cho chủ bài viết (nếu người comment không phải là chủ bài viết)
            if (post.UserId != userId)
            {
                // Nếu là reply, ta có thể chọn không thông báo cho chủ bài viết để tránh spam,
                // hoặc vẫn thông báo. Ở đây ta giữ nguyên là vẫn thông báo "đã bình luận".
                // Tuy nhiên, nếu user vừa reply vào comment CỦA CHÍNH CHỦ BÀI VIẾT, thì 
                // thông báo "đã trả lời bình luận của bạn" bên dưới sẽ được gửi.
                // Ta nên tránh gửi 2 thông báo cùng lúc.
                
                bool shouldNotifyOwner = true;
                if (dto.ParentCommentId.HasValue)
                {
                    var parent = await _context.Comments.FindAsync(dto.ParentCommentId.Value);
                    if (parent != null && parent.UserId == post.UserId) 
                    {
                        // Nếu reply vào comment của chủ bài viết -> sẽ được xử lý ở bước 2 -> bỏ qua bước 1
                ...
                if (shouldNotifyOwner)
                {
                    ...
                    var noti = await CreateNotification(post.UserId, userId.Value, dto.PostId, "comment", message);
                    await SendNotificationToClient(post.UserId, noti, user);
                }
            }

            // 2. Gửi thông báo cho người được trả lời
            int? notifyTargetId = null;
            if (dto.TargetUserId.HasValue) 
            {
                 notifyTargetId = dto.TargetUserId.Value;
            }
            else if (dto.ParentCommentId.HasValue)
            {
                var parentComment = await _context.Comments.FindAsync(dto.ParentCommentId.Value);
                if (parentComment != null) notifyTargetId = parentComment.UserId;
            }

            if (notifyTargetId.HasValue && notifyTargetId.Value != userId)
            {
                 string msg = $" đã trả lời bình luận của bạn: {dto.Content}";
                 var noti = await CreateNotification(notifyTargetId.Value, userId.Value, dto.PostId, "reply", msg); 
                 await SendNotificationToClient(notifyTargetId.Value, noti, user);
            }
            ...
            return Json(new
            {
                ...
                canDelete = true,
                parentCommentId = dto.ParentCommentId // Trả về để FE biết đây là reply
            });
        }

        private async Task<Notification> CreateNotification(int toUserId, int fromUserId, int postId, string type, string message)
        {
            var notification = new Notification
            {
                UserId = toUserId,
        ...
        private async Task SendNotificationToClient(int toUserId, Notification n, User? fromUser)
        {
            var data = new
            {
                Id = n.Id,
            ...
            await _hubContext.Clients.Group(toUserId.ToString()).SendAsync("ReceiveNotification", data);
        }
        ...
        public async Task<IActionResult> DeleteComment([FromBody] int commentId)
        {
            ...
            try
            {
                // 1. Manually delete replies first (Manual Cascade)
                var replies = _context.Comments.Where(c => c.ParentCommentId == commentId).ToList();
                if (replies.Any())
                {
                ...
                var newCount = _context.Comments.Count(c => c.PostId == comment.PostId);
                return Json(new { success = true, comments = newCount });
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> GetComments(int postId)
        {
            ...
            return PartialView("_CommentList", post);
        }
    }
}
