...
public class ChatHub : Hub
{
   ...
   public override Task OnConnectedAsync()
    ...
    public async Task<int> InitiateVideoCall(int conversationId, int targetUserId, bool audioOnly = false)
    {
        var userId = _httpContextAccessor.HttpContext?.Session.GetInt32("UserId");
        if (userId == null) throw new HubException("Not authenticated");
        ...
        // Get Caller Info (Avatar)
        var caller = await _db.Users.FindAsync(userId.Value);
        var callerAvatar = string.IsNullOrEmpty(caller?.ImageUrl) ? "/Uploads/default-avatar.png" : $"/Uploads/{caller.ImageUrl}";
    ...
    public async Task AcceptVideoCall(int conversationId, int callerId)
    {
        ...
        // Get Answerer Info (Avatar)
        var answerer = await _db.Users.FindAsync(userId.Value);
        var answererAvatar = string.IsNullOrEmpty(answerer?.ImageUrl) ? "/Uploads/default-avatar.png" : $"/Uploads/{answerer.ImageUrl}";
    ...
    public async Task ToggleCamera(int conversationId, bool isEnabled)
    {
        var userId = _httpContextAccessor.HttpContext?.Session.GetInt32("UserId");
        ...
        if (_activeCallParticipants.TryGetValue(conversationId, out var participants))
        {
            ...
            lock (participants)
            {
                recipients = participants.Where(p => p != userId.Value).ToList();
            }
            ...
            foreach (var recipientId in recipients)
            {
                if (_connections.TryGetValue(recipientId, out var conns))
                {
                    ...
                    foreach (var connId in connectionsToNotify)
                    {
                        await Clients.Client(connId).SendAsync("ReceiverToggleCamera", new 
                        {
                            userId = userId.Value,
                            isEnabled = isEnabled
                        });
