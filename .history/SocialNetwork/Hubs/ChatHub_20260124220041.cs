...
public class ChatHub : Hub
{
   ...
   public override Task OnConnectedAsync()
    ...
    {
        var userId = _httpContextAccessor.HttpContext?.Session.GetInt32("UserId");
        // Fallback to Context.Items if session is null (sometimes happens in SignalR if not configured perfectly, but usually Session works)
        ...
        if (unreadMessages.Any())
        {
            ...
            {
                 await Clients.Users(senderIdStrings).SendAsync("MessagesRead", new 
                 {
                     conversationId = conversationId,
                     readerId = readerId,
        ...
        // Calculate Total Unread Count to return
         var totalUnread = await _db.Conversations
            ...
            .ToListAsync(); // Pull to memory to calculate complex logic or try to do it in SQL if possible (EF Core 3.1+ can be tricky with complex conditions)

        // Doing calculation in memory to match Controller logic safely
        int count = 0;
        foreach(var c in totalUnread)
        {
             if (c.IsGroup)
             {
                 ...
                 if (p == null) continue;
                 // Logic: Message > JoinedAt AND (Message > DeletedAt OR DeletedAt is null)
